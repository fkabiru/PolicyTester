...
const LoansStatement = ({ userRoles }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      updateLoanParams(userParams); // Populate {accObject} object with the current account

      LoansSmtAPI(userParams)
        .then((lnstm) => {
          if (lnstm.status == "400") {
            setsubmitting(false);
            return window.alert(`${lnstm.error} `);
          }
          setBase64(lnstm.base64);
  ...
  const updateLoanParams = (accParams) => {
    let accPrms = {
  ...
  //Check user rights ,whether the user is admin or reports
  const userRights = () => {
    let userRoles = JSON.parse(localStorage.getItem("userRoles"));
    if (userRoles.includes("Admin")) return "Admin";
    if (userRoles.includes("Reports")) return "Reports";
  };
